...
// working fucking mod
let wfm a b = 
    if b = 0L then failwith "infinite loop"
    ((a % b) + b) % b

let rec compatible (mult1, offset1) (mult2, offset2) =

    if mult1 < mult2 then compatible (mult2, offset2) (mult1, offset1) else

    let lcm = lcm mult1 mult2

    let start = wfm offset2 mult1
    let step = wfm mult2 mult1
    let count = lcm / mult2
    { 0L .. count - 1L }
    |> Seq.tryFind (fun i -> wfm (i * step + start) mult1 = offset1)
    |> Option.map (fun i -> let collision = i * mult1 + offset2 in (lcm, wfm collision lcm))
...
let verify_vel (position_velocities: (int64 * int64) array) (candidate_vel: int64) =
    ...
    seq {
        for pz, vz in position_velocities do
            ...
            if vdelta = 0 then
                ...
                let m = (vdelta, wfm pz vdelta)
                printfn "Hailstone at %i moving at %i vs our hailstone with velocity %i" pz vz candidate_vel
                printfn "Start position has to be multiple of %i plus %i" (fst m) (snd m)
                match compatible current m with
                | Some next -> 
                    current <- next
                | None -> yield None
    ...
    |> Seq.head
    |> Option.map (fun x -> { Velocity = candidate_vel; Solution = x })
